package com.systeminfos.common.mybatisplus.generator;
...
public class MybatisPlusGenerator {
    ...
    public static void main(String[] args) {
        ...
        String packageName = "com.systeminfos.summer.order";
        //表名  表名（英文逗号分隔）
        List<String> tableNames = new ArrayList<>();
        tableNames.add("summer_order");

        for (String tableName : tableNames) {
            generator(author, date, moduleName, packageName, tableName, entityName);
        }
    ...
    private static void generator(String author, String date, String moduleName, String packageName, String tableNames, String entityName) {
        ...
        // 代码生成器
        // mysql
        new AutoGenerator(configBuilder(new DataSourceConfig.Builder("jdbc:mysql://localhost:3306/systeminfos_summer?useSSL=false&useUnicode=true&characterEncoding=UTF-8&serverTimezone=UTC", "root", "mysqlmysql")))
                // 全局配置
                ...
                // 模板配置
                .template(configBuilder(new TemplateConfig.Builder().entity("/entity.java").mapper("/mapper.java").mapperXml("/mapper.xml").service("/service.java", "/serviceImpl.java").controller(null)))
                // 执行并指定模板引擎
